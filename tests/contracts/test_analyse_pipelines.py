"""Tests für die Analyse-Pipelines-Verträge — Buy-Signal (Story S-028).

Covers (analyse-pipelines): AC2

`app.contracts.analyse_pipelines` bildet den Output-Vertrag "(a) Output
neue Titel" aus `docs/specs/analyse-pipelines.md` ab: `BuySignal` enthält
mindestens Titel + Gesamtscore (AC2) plus Kategorie-Scores, geerdete
Fakten und Zeitstempel. Diese Tests decken auf DTO-Ebene die Struktur
(Pflichtfelder, Wertebereich, Immutabilität); die eigentliche
Buy-Signal-Erzeugung (AC1, AC3, AC4) liegt in
`tests/domain/analysis_new/test_buy_pfad.py`.
"""
