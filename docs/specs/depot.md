...
- **AC6** — Für Positionen in Fremdwährung wird der G/V in eine Kapital- und eine Währungskomponente zerlegt und beide in CHF-Basiswährung ausgewiesen (FX-Attribution, deckt A3). Präzisiert in S-053: realisiert (je Verkauf) ist `kapital_gv_chf = (erloes_netto − ø_einstand × verkaufte_menge) × ø_einstand_fx_rate` und `waehrungs_gv_chf = erloes_netto × (verkauf_fx_rate − ø_einstand_fx_rate)`; unrealisiert (je offener Position) ist `fx_kapital_gv = (aktueller_preis − ø_einstand) × menge × ø_einstand_fx_rate` und `fx_waehrungs_gv = aktueller_preis × menge × (aktueller_fx_rate − ø_einstand_fx_rate)`. Beide Summen ergeben exakt den gesamten G/V in CHF (kein unattribuierter Rest). Bei `waehrung == CHF` bleiben alle vier Werte `None` (keine Attribution nötig).
...
- **AC12** — Die Fill→Position-Orchestrierung ist Sache dieses Moduls: es konsumiert das Ausführungsergebnis (inkl. Pflichtfeld `mode` und `client_order_id`) und schreibt Position + Transaktion im selben Modus; ein Fill ohne `mode` oder ohne auflösbare Strategie-Katalog-ID (bei Kauf) wird nicht gebucht, sondern als fehlerhaft protokolliert (deckt E1). Simulierte Fills erzeugen ausschliesslich simulierte Positionen/Transaktionen.

> **Traceability:** Jeder Test trägt das kanonische Trace-Tag `@trace depot#AC<n>`.

## Verträge

- **Input (Ausführungsergebnis, vom Kauf-/Verkaufsmodul):** `{ client_order_id, titel_id, anlageklasse (1–11), gics_branche, richtung (kauf|verkauf), menge, fill_preis, kosten (gebuehren), arrival_price, waehrung, fx_rate, zeitstempel, mode (echt|simuliert) }`; bei Kauf zusätzlich das beim Kauf fixierte Attribut-Bündel `{ strategie, zeithorizont, exit_regeln, these }` — die Strategie wird ausschliesslich über ihre Katalog-ID aus [[strategie-exit-regeln]] referenziert (keine Namens-Zuordnung; Anzeige-Name kommt aus dem Strategie-Katalog); das Depot-Contract-Feld heisst aktuell `strategie` und trägt diese Katalog-ID (Umstellung auf explizites `strategie_id` ist Teil der Attribut-Bündel-Fixierung, S-040). (`mode` präzisiert in S-016: jede transaktionale Entität trägt den Modus, → BR-130; er stammt aus der Order-Ausführung selbst, nicht aus einer Depot-internen Konfiguration.) (`client_order_id` präzisiert in S-016: universelles Pflichtfeld und eindeutiger Dedup-Schlüssel je Fill — die Redis-Queue liefert Fills at-least-once (P8), das Depotmodul verbucht denselben `client_order_id`-Wert nie ein zweites Mal gegen den Bestand, → ADR-011.) (`fx_rate` präzisiert in S-053: Kurs zur CHF-Basiswährung zum Zeitpunkt des Fills — Pflicht bei `waehrung != CHF` (sonst kann keine FX-Attribution, AC6, berechnet werden), bei CHF `None`, → BR-129.)
- **Mode-Isolation (← C-016, Datenmodell BR-113/BR-130):** `mode` ist Pflichtfeld; Positionen, Transaktionen und Aggregate werden strikt je Modus getrennt geführt. Ein simulierter Fill verändert nie einen Echt-Bestand (und umgekehrt); alle Ausgaben an Risikomanagement/Überwachung/Dashboard sind modus-gefiltert.
- **Position (interner Zustand):** `{ titel_id, anlageklasse, gics_branche, menge, ø_einstand, ø_einstand_fx_rate, aktuelle_bewertung, strategie, zeithorizont, exit_regeln, these, realisierter_gv, unrealisierter_gv, fx_kapital_gv, fx_waehrungs_gv }`. (`ø_einstand_fx_rate` präzisiert in S-053: der Ø-Einstands-FX-Kurs — analog zu `ø_einstand`, mengen-gewichtet gemittelt bei Nachkauf — als Berechnungsgrundlage für `fx_kapital_gv`/`fx_waehrungs_gv`; `None` bei einer CHF-Position. `fx_kapital_gv`/`fx_waehrungs_gv` selbst sind unrealisierte FX-Attribution — analog zu `unrealisierter_gv` reine, noch nicht an eine Bewertungsschleife angebundene Formelwerte, siehe AC6.)
- **Transaktionshistorie (append-only):** Liste von `{ trade_id, titel_id, richtung, menge, fill_preis, arrival_price, slippage, kosten, waehrung, zeitstempel, fx_rate, kapital_gv_chf, waehrungs_gv_chf }`. (`fx_rate, kapital_gv_chf, waehrungs_gv_chf` präzisiert in S-053: `fx_rate` je Trade in Fremdwährung (Kauf UND Verkauf), `kapital_gv_chf`/`waehrungs_gv_chf` (realisierte FX-Attribution, AC6) nur je Verkauf-Trade in Fremdwährung — alle drei `None` bei CHF, → BR-129.)
...
- **Output an Depot-Überwachung:** je Titel `{ titel_id, anlageklasse (1–11), strategie, exit_regeln }`. (`anlageklasse` präzisiert in S-032: `[[depot-ueberwachung]]` AC1 benötigt sie zur Bestimmung der je Klasse überwachten Grössen — bereits als `PositionsBestand.asset_class_id` vorhanden, hier für den Depot-Überwachung-Konsumenten ergänzt.)
