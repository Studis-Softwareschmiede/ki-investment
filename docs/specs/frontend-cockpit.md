...
- **AC33** — **Depot-Verlauf-Chart in der Depot-View:** Die Depot-View zeigt den Portfolio-Verlauf als Zeitreihen-Chart (§8.2, Zeit auf X, Portfolio-Wert auf Y; End-Delta trägt §2.3-G/V-Kodierung, nicht nur Farbe → D3) aus dem Snapshot-Read-Modell (AC32). **Default-Rendering = server-gerendertes inline-SVG (lib-frei)** — analog zum Spinnennetz (§8.1), robust/offline/Playwright-testbar, ohne Bundle (ADR-012/013). Die vendored Canvas-Lib **uPlot** (§8.2 Empfehlung) ist **nur** bei expliziter Owner-/designer-Freigabe zulässig (offene Entscheidung, siehe Nicht-Ziele) — ohne Freigabe bleibt es beim server-SVG. Leerer/kurzer Verlauf → definierter Empty-State („noch kein Verlauf im Modus SIMULIERT"), nie ein Fake-Wert. Chart-Container mit fester `--min-height` (kein Layout-Sprung); `prefers-reduced-motion` schaltet die Einzeichen-Animation ab (§8.2/css R03). Stabile `data-testid` gemäss `design.md` §7/§8. Der Demo-Seed (AC22) füllt das Snapshot-Read-Modell deterministisch + order-frei, sodass der Chart gefüllt und Playwright-testbar ist.
...
- **Keine konkrete Chart-Lib-Datei-Freigabe** — Spinnennetz server-SVG (keine Lib); Zeitreihen-Empfehlung uPlot (Freigabe Owner/designer). Der **Depot-Verlauf-Chart** (AC33) rendert per Default lib-frei als server-SVG; die uPlot-Freigabe bleibt eine **offene Owner-/designer-Entscheidung** und ist in Story S-081 als solche vermerkt.
...
- Depotmodul + Scheduler (`[[depot]]` / `[[scheduler]]`) + `dba` — Portfolio-Wert-Snapshot-Read-Modell + periodischer Aufzeichnungs-Job (AC32/AC33, Story S-081). **Offen (Owner/designer):** Zeitreihen-Chart-Lib-Freigabe (uPlot vs. server-SVG-Default).
